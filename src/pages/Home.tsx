...
export default function Home() {
  ...
  const translatedDestinations: Destination[] = appData.destinations.map(d => ({
    ...d,
    price: Number(t(`destinations.${d.id}.price`)),
    title: t(`destinations.${d.id}.title`),
    ...
    <>
      ...
      <header className="relative h-screen flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full">
          <motion.div
            ...
            className="max-w-2xl"
          >
            <span className="inline-block bg-orange-600 text-white px-4 py-1 rounded-full text-sm font-bold tracking-widest uppercase mb-6">
              {t('company.tagline')}
            </span>
            <h1 className={`${isXY ? 'text-8xl md:text-9xl' : 'text-6xl md:text-8xl'} font-display font-bold text-white leading-[0.9] mb-8`}>
              <Trans 
                i18nKey="hero.title"
                components={{
      ...
      <section id="destinations" className="py-32 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <div>
            ...
            <p className="text-stone-500 text-lg md:text-xl whitespace-pre-line">
              <Trans 
          ...
          <div className="flex bg-stone-100 p-1 rounded-2xl">
            {['all', 'surf', 'bike', 'climb'].map((tab) => (
              <button
                ...
                className={`px-6 py-2 rounded-xl text-sm font-bold transition-all capitalize ${activeTab === tab ? 'bg-white shadow-sm text-orange-600' : 'text-stone-400 hover:text-stone-600'}`}
              >
                {t(`common.${tab}`)}
              </button>
            ))}
          ...
          <AnimatePresence mode="popLayout">
            {filteredDestinations.map((dest) => (
              <motion.div
                ...
                className="group relative bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-stone-100 flex flex-col h-full"
              >
                ...
                <div className="p-8 flex-1 flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="text-3xl font-display font-bold text-orange-600">
                      {t('common.currencySymbol')}{dest.price}<span className="text-sm text-stone-400 font-sans font-normal ml-1">{t('common.perPerson')}</span>
                    </div>
                  </div>
                  ...
                  <Link to={`/destination/${dest.id}`} className="w-full py-5 rounded-2xl bg-stone-50 text-stone-900 font-bold text-lg flex items-center justify-center gap-2 hover:bg-stone-900 hover:text-white transition-all group/btn mt-auto">
      ...
      <section id="about" className="py-32 bg-stone-900 text-white overflow-hidden relative">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div>
            <span className="text-orange-500 font-bold tracking-widest uppercase text-base mb-6 block">{t('common.ourPhilosophy')}</span>
            <h2 className="text-4xl md:text-6xl font-display font-bold mb-8 leading-tight">{t('common.guideTitle')}</h2>
        ...
        <div className="glass-card rounded-[40px] p-8 md:p-16 grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div className="space-y-8">
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <div className="text-xl font-bold">{t('common.phoneNumber')}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
              ...
              {isXY ? (
                <>
                  <div className="space-y-2">
                    ...
                    <input type="text" className="w-full bg-stone-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-orange-500 transition-all" placeholder={t('common.placeholderLast')} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-widest text-stone-400">{t('common.firstName')}</label>
                    <input type="text" className="w-full bg-stone-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-orange-500 transition-all" placeholder={t('common.placeholderFirst')} />
                  </div>
                </>
              ) : (
                <>
            ...
            <div className="space-y-2">
              ...
              <input type="email" className="w-full bg-stone-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-orange-500 transition-all" placeholder={t('common.placeholderEmail')} />
            </div>
            <div className="space-y-2">
            ...
            <button className="w-full bg-orange-600 text-white py-5 rounded-2xl font-bold text-lg hover:bg-orange-700 transition-all shadow-lg shadow-orange-600/20">
